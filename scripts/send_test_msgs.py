...
roslib.load_manifest("spencer_tracking_rviz_plugin")
import copy
import random
...
def createTrackAndDetection(tracks, detections, track_id, detection_id, angle, radius):
    ...
    if detection_id >= 0:
        ...
        detectedPerson.pose.pose.position.x += (
            random.random() * 0.5 - 0.25
        )  # introduce some noise on observation position
...
while not rospy.is_shutdown():
    br.sendTransform(
        frameOffset, frameOrientation, rospy.Time.now(), "test_tf_frame", "odom"
    )
    ...
    createTrackAndDetection(
        tracks, detections, idShift + 1, 7, currentAngle + pi / 2, 2.5
    )
    createTrackAndDetection(
        tracks, detections, idShift + 2, -1, currentAngle + pi / 1, 3.0
    )
    createTrackAndDetection(
        tracks,
        detections,
